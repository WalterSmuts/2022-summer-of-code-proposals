...
Since I've outlined two different approaches for implementing an XR fish tank
effect it makes sense to have two applicants work in parallel, each on a separate
approach. This could be described as two DISTINCT projects with a common goal. I
believe there is little overlap in the actual coding work yet lots of overlap in
theory. The advantage of this is that the applicants could support each other in
the theory while still having complete ownership of their own project.

I'd be happy to work on either avenue of implementing the fish tank effect if
this suggestion is viable. In my mind, both are worth doing.
